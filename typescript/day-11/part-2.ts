...
const parseMonkey = (monkey: string): Monkey => {
  ...
  const inspections = 0;
  const divisor = testValue;

  return { id, items, operation, test, inspections, divisor };
};
...
// const getsBored = (val: number) => val;


// const DIVISOR = 96577; // test
const DIVISOR = 9699690; // real
const getsBored = (val: number) => val % DIVISOR;
...
const ROUNDS = 10000;
const finalFamily = runRounds(family, ROUNDS);
...
console.log("\nDivisors:", uniqueDivisors);

const product = (nums: number[]) => nums.reduce((acc, num) => acc * num, 1);
const congruence = product(uniqueDivisors);
